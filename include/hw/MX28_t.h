...
namespace drwn {
    ...
    public:
        ...
        static int get_mirror_value(int value) { return MAX_VALUE + 1 - value; }

        static float get_mirror_angle(float angle) { return -angle; }

        static int angle_2_value(float angle) { return (int) (angle * RATIO_DEGREES2VALUE) + CENTER_VALUE; }

        static float value_2_angle(int value) { return (float) (value - CENTER_VALUE) * RATIO_VALUE2DEGREES; }

        static float speed_2_value(float speed) {
            int temp = ((int) (fabs(speed) * RATIO_SPEED2VALUE)) & 0x3FF;
            if (speed < 0) temp |= 0x400;
            return temp;
        }

        static float value_2_speed(int value) {
            float temp = (value & 0x3FF) * RATIO_VALUE2SPEED;
            if (value & 0x400) temp = -temp;
            return temp;
        }

        static float torque_2_value(float speed) {
            int temp = ((int) (fabs(speed) * RATIO_TORQUE2VALUE)) & 0x3FF;
            if (speed < 0) temp |= 0x400;
            return temp;
        }

        static float value_2_torque(int value) {
