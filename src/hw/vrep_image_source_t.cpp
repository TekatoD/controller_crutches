...
int vrep_image_source_t::get_client_id() const {
    return m_client_id;
}

void vrep_image_source_t::set_client_id(int client_id) {
    if(client_id != -1) {
...
vrep_image_source_t::vrep_image_source_t(std::string sensor_name, int width, int height)
        : m_sensor_name(std::move(sensor_name)), m_width{width}, m_height{height} {
}

void vrep_image_source_t::connect() {
    if(m_client_id != -1) {
...
bool vrep_image_source_t::is_debug_enabled() const {
    return m_debug;
}

void vrep_image_source_t::enable_debug(bool debug) {
    m_debug = debug;
}
